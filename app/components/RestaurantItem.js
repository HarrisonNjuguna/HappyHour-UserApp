            ...
            <View style={tailwind`flex-row items-center mt-1`}>
                <View style={tailwind`flex-grow`}>
                    ...
                    <View style={tailwind`flex-row items-center`}>
                        <MaterialCommunityIcons name="clock-time-four" size={13} color="#ff0000" />
                        <Text style={tailwind`text-xs text-gray-700`}> 20-30 • min • Ksh. {item.price.toFixed(0)}</Text>   
                    </View>
                </View>
                <View style={tailwind`w-8 h-8 justify-center items-center bg-gray-100 rounded-full`}>
                    <Text style={tailwind`text-gray-600 text-xs`}>{item.rating.toFixed(1)}</Text>
                </View>
            </View>
        </TouchableOpacity>
    )
